Feature: Sales Module UI Testing
  ...
  @UI_SALES_PAG_ADM @Admin @UI @215542N
  Scenario: UI_SALES_PAG_ADM - Check sales list pagination and default sorting
  ...
  @UI_SALES_SORT_ADM @Admin @UI @215542N
  Scenario: UI_SALES_SORT_ADM - Test column sorting functionality
    Given I log in as Admin with username "admin" and password "admin123"
  ...
  @UI_SELL_PLANT_ADM @Admin @UI @215542N
  Scenario: UI_SELL_PLANT_ADM - Test selling a plant (Happy path)
    Given I log in as Admin with username "admin" and password "admin123"
  ...
  @UI_SELL_PLANT_VALID_ADM @Admin @UI @215542N
  Scenario: UI_SELL_PLANT_VALID_ADM - Test validation messages for selling a plant
    Given I log in as Admin with username "admin" and password "admin123"
    And at least one plant has stock
  ...
  @UI_DELETE_SALE_ADM @Admin @UI @215542N
  Scenario: UI_DELETE_SALE_ADM - Test Delete Sale confirmation
    Given I log in as Admin with username "admin" and password "admin123"
    And I navigate to the sales list page
  ...
  @UI_SALES_READONLY_USR @User @UI @215542N
  Scenario: UI_SALES_READONLY_USR - Check read-only access to sales list
  ...
  @UI_SALES_EMPTY_USR @User @UI @215542N
  Scenario: UI_SALES_EMPTY_USR - Check 'No sales found' empty state
  ...
  @UI_ADMIN_BTN_HIDDEN_USR @User @UI @215542N
  Scenario: UI_ADMIN_BTN_HIDDEN_USR - Check if Admin buttons are hidden
    Given I log in as User with username "testuser" and password "test123"
    And there is at least one sale record
    When I navigate to the sales list page
  ...
  @UI_FORCE_URL_403_USR @User @UI @215542N
  Scenario: UI_FORCE_URL_403_USR - Test 403 redirect by forcing URL
    Given I log in as User with username "testuser" and password "test123"
    When I force navigate to "/ui/sales/new"
    Then I should be redirected to the 403-Access Denied page

  @UI_SALES_SORT_USR @User @UI @215542N
  Scenario: UI_SALES_SORT_USR - Test if grid sorting works for normal users
    Given I log in as User with username "testuser" and password "test123"
    And I navigate to the sales list page
    And there is at least one sale record
    When I click on the "Plant name" column header

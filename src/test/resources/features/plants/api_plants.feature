Feature: Plants Module API Testing
  ...
  Scenario: T-API-21 - Test POST /api/plants/category/{categoryId} endpoint as Admin
    ...
    Then the plants API response status code should be 201
  ...
  Scenario: T-API-22 - Test POST /api/plants with plant name shorter than 3 characters
    Given I am authorized as Admin for the plants API
    And I have a valid sub-category ID for plant creation
    When I send a POST request to create a plant with name "AB" price 50.00 and quantity 10
    Then the plants API response status code should be 400
  ...
  Scenario: T-API-23 - Test POST /api/plants with price equal to zero
    Given I am authorized as Admin for the plants API
    And I have a valid sub-category ID for plant creation
  ...
  Scenario: T-API-24 - Test PUT /api/plants/{id} updates an existing plant successfully
    Given I am authorized as Admin for the plants API
  ...
  Scenario: T-API-25 - Test DELETE /api/plants/{id} removes the plant
    Given I am authorized as Admin for the plants API
  ...
  Scenario: T-API-26 - Test GET /api/plants as User
  ...
  Scenario: T-API-27 - Test GET /api/plants/{id} as User
    Given I am authorized as User for the plants API
  ...
  Scenario: T-API-28 - Test GET /api/plants/paged as User
    Given I am authorized as User for the plants API
  ...
  Scenario: T-API-29 - Test POST /api/plants/category/{categoryId} as User returns 403 Forbidden
    Given I am authorized as User for the plants API
    And I have a valid sub-category ID for plant creation
  ...
  Scenario: T-API-30 - Test DELETE /api/plants/{id} as User returns 403 Forbidden
    Given I am authorized as User for the plants API
    And I have a valid plant ID to retrieve via API
    When I send a DELETE request to remove the plant by ID
    Then the plants API response status code should be 403

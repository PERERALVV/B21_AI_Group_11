Feature: Admin & User UI – Category Management
  ...
  Scenario: TC-UI-CAT-A01 Verify that Admin can view the category list
  ...
  Scenario: TC-UI-CAT-A02 Verify that Admin can add a new main category with valid name
    When the admin opens the Category List page
  ...
  Scenario: TC-UI-CAT-A03 Verify that Validation error when category name is empty
  ...
  Scenario: TC-UI-CAT-A04 Verify that Validation error when name is shorter than 3 characters
    When the admin opens the Add Category page
  ...
  Scenario: TC-UI-CAT-A05 Verify that Admin can delete a category - no confirmation dialog expected per SRS
  ...
  Scenario: TC-UI-CAT-U01 Verify that User can view category list in read-only mode
  ...
  Scenario: TC-UI-CAT-U02 Verify that User can search categories by name
    Given a category named "Roses" exists in the system
    When the user opens the Category List page
    And the user searches for "Roses"
    Then validate only categories matching "Roses" are shown

  @UI @User @TC-UI-CAT-U03
  Scenario: TC-UI-CAT-U03 Verify that User can filter categories by parent
    When the user opens the Category List page
    And the user filters by parent "Flowers"
    Then validate the category list is displayed with at least one record

  @UI @Admin @TC-UI-CAT-U04
  Scenario: TC-UI-CAT-U04 Verify that Cancel button returns user to category list
    When the admin opens the Add Category page
    And the user clicks Cancel
    Then validate the user is on the Category List page

  @UI @User @TC-UI-CAT-U05
  Scenario: TC-UI-CAT-U05 Verify that User can sort category list by Name
    When the user opens the Category List page

Feature: Sales Module API Testing
  ...
  @T-API-GET-SL-ADM-1 @Admin @API @215542N
  Scenario: T-API-GET-SL-ADM-1 - Check GET /api/sales endpoint with Admin token
  ...
  @T-API-POST-SL-ADM-2 @Admin @API @215542N
  Scenario: T-API-POST-SL-ADM-2 - Test POST /api/sales/plant/{plantId} (Happy path)
    Given I have a valid Admin authorization token
  ...
  @T-API-POST-SL-ADM-3 @Admin @API @215542N
  Scenario: T-API-POST-SL-ADM-3 - Test POST /api/sales/plant/{plantId} with quantity 0
    Given I have a valid Admin authorization token
    And I have a valid plant ID with stock
  ...
  @T-API-GET-SL-ADM-4 @Admin @API @215542N
  Scenario: T-API-GET-SL-ADM-4 - Check GET /api/sales/{id} endpoint
    Given I have a valid Admin authorization token
  ...
  @T-API-DELETE-SL-ADM-5 @Admin @API @215542N
  Scenario: T-API-DELETE-SL-ADM-5 - Check DELETE /api/sales/{id} endpoint
    Given I have a valid Admin authorization token
  ...
  @T-API-GET-SL-USR-1 @User @API @215542N
  Scenario: T-API-GET-SL-USR-1 - Check GET /api/sales with normal user token
  ...
  @T-API-GET-SL-USR-2 @User @API @215542N
  Scenario: T-API-GET-SL-USR-2 - Check GET /api/sales/page with pagination
    Given I have a valid User authorization token
  ...
  @T-API-GET-SL-USR-3 @User @API @215542N
  Scenario: T-API-GET-SL-USR-3 - Check GET /api/sales/{id} with normal user token
    Given I have a valid User authorization token
    And there is at least one sale record in the database
    And I have a valid sale ID
    When I send a GET request to "/api/sales/{id}" for that sale ID
  ...
  @T-API-POST-SL-USR-4 @User @API @215542N
  Scenario: T-API-POST-SL-USR-4 - Test 403 Forbidden on POST (Security)
    Given I have a valid User authorization token
    And I have a valid plant ID with stock
    When I send a POST request to "/api/sales/plant/{plantId}" with quantity 1
    Then the API response status code should be 403

  @T-API-DELETE-SL-USR-5 @User @API @215542N
  Scenario: T-API-DELETE-SL-USR-5 - Test 403 Forbidden on DELETE (Security)
    Given I have a valid User authorization token
    And there is at least one sale record in the database
    And I have a valid sale ID
    When I send a DELETE request to "/api/sales/{id}" for that sale ID

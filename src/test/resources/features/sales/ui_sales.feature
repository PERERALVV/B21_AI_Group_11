Feature: Sales Module UI Testing
  ...
  @T-UI-1 @Admin @UI @215542N @bug
  Scenario: T-UI-1 - Verify that Check sales list pagination and default sorting
  ...
  Scenario: T-UI-2 - Verify that Test column sorting functionality
    Given I log in as Admin with username "admin" and password "admin123"
    ...
    Then validate the sales list should be sorted by "Plant name" ascending
    When I click on the "Plant name" column header again
    Then validate the sales list should be sorted by "Plant name" descending
    When I click on the "Quantity" column header
    Then validate the sales list should be sorted by "Quantity" ascending
    When I click on the "Total price" column header
    Then validate the sales list should be sorted by "Total price" ascending
    When I click on the "Sold date" column header
    Then validate the sales list should be sorted by "Sold date" ascending

  @T-UI-3 @Admin @UI @215542N
  Scenario: T-UI-3 - Verify that Test selling a plant (Happy path)
    Given I log in as Admin with username "admin" and password "admin123"
    And at least one plant has stock
    When I click the "Sell Plant" button
    Then validate I should see the "Sell Plant" form
  ...
  Scenario: T-UI-4 - Verify that Test validation messages for selling a plant
    Given I log in as Admin with username "admin" and password "admin123"
    And at least one plant has stock
    And I navigate to the "Sell Plant" page
    When I click the "Sell" button without selecting a plant
    Then validate I should see the validation message "Plant is required" for the plant field
    When I select a plant with stock
    And I enter a quantity of 0
    And I click the "Sell" button
    Then validate I should see the validation message "Value must be greater than 0" for the quantity field

  @T-UI-5 @Admin @UI @215542N
  Scenario: T-UI-5 - Verify that Test Delete Sale confirmation
    Given I log in as Admin with username "admin" and password "admin123"
    And I navigate to the sales list page
    And there is at least one sale record
    When I click the delete button on the first sale record
    Then validate I should see a confirmation prompt asking "Are you sure you want to delete this sale?"
    When I confirm the deletion prompt
    Then validate the sale record should be removed from the list

  @T-UI-6 @User @UI @215542N
  Scenario: T-UI-6 - Verify that Check read-only access to sales list
  ...
  Scenario: T-UI-7 - Verify that Check 'No sales found' empty state
    Given the database has 0 sales records
    And I log in as User with username "testuser" and password "test123"
    When I navigate to the sales list page
    Then validate I should see the text "No sales found" in the table

  @T-UI-8 @User @UI @215542N
  Scenario: T-UI-8 - Verify that Check if Admin buttons are hidden
    Given I log in as User with username "testuser" and password "test123"
    And there is at least one sale record
    When I navigate to the sales list page
    Then validate the "Sell Plant" button should not be visible
    And the "Delete" action buttons should not be visible for any record

  @T-UI-9 @User @UI @215542N
  Scenario: T-UI-9 - Verify that Test 403 redirect by forcing URL
    Given I log in as User with username "testuser" and password "test123"
  ...
  Scenario: T-UI-10 - Verify that Test if grid sorting works for normal users
    Given I log in as User with username "testuser" and password "test123"
    And I navigate to the sales list page
    And there is at least one sale record
    When I click on the "Plant name" column header

Feature: Plants Module UI Testing
  ...
  Scenario: T-UI-21 - Verify that Admin views Plants page with all management controls visible
    Given I log in as Admin with username "admin" and password "admin123"
    When I navigate to the plants list page
    Then validate the "Add a Plant" button should be visible on the plants page
  ...
  Scenario: T-UI-22 - Verify that Admin successfully adds a new plant with valid data
    Given I log in as Admin with username "admin" and password "admin123"
  ...
  Scenario: T-UI-23 - Verify that Add Plant form shows field-level validation errors for invalid inputs
    Given I log in as Admin with username "admin" and password "admin123"
    When I navigate to the add plant page directly
    And I enter plant price "0"
    And I enter plant quantity "-1"
    And I click the Save button on the plant form
    Then validate I should see plant name validation error containing "Plant name is required"
  ...
  Scenario: T-UI-24 - Verify that Admin edits a plant and the Low badge appears when quantity drops below 5
    Given I log in as Admin with username "admin" and password "admin123"
  ...
  Scenario: T-UI-25 - Verify that Admin deletes a plant via confirmation prompt
    Given I log in as Admin with username "admin" and password "admin123"
    And at least one plant exists in the system
    When I navigate to the plants list page
    And I note the current plant count
    And I click the Delete icon on the first plant in the list
    Then validate I should see a plant deletion confirmation prompt with text "Delete this plant?"
  ...
  Scenario: T-UI-26 - Verify that User views Plants page and all management controls are hidden
    Given I log in as User with username "testuser" and password "test123"
    When I navigate to the plants list page
    Then validate the "Add a Plant" button should not be visible on the plants page
  ...
  Scenario: T-UI-27 - Verify that User searches for a plant by name
    Given I log in as User with username "testuser" and password "test123"
  ...
  Scenario: T-UI-28 - Verify that User filters plants by category
    Given I log in as User with username "testuser" and password "test123"
  ...
  Scenario: T-UI-29 - Verify that User sorts plant list by Name, Price, and Stock ascending then descending
    Given I log in as User with username "testuser" and password "test123"
    And at least one plant exists in the system
    When I navigate to the plants list page
    And I click on the plants table "Name" column header
    Then validate the plant list should be sorted by "Name" ascending
    When I click on the plants table "Name" column header again
    Then validate the plant list should be sorted by "Name" descending
    When I click on the plants table "Price" column header
    Then validate the plant list should be sorted by "Price" ascending
    When I click on the plants table "Stock" column header
    Then validate the plant list should be sorted by "Stock" ascending

  @T-UI-30 @User @UI @215533M
  Scenario: T-UI-30 - Verify that User directly navigates to /ui/plants/add and is denied access
    Given I log in as User with username "testuser" and password "test123"
  ...
  Scenario: T-UI-31 - Verify that Plants table fourth column header should be named Quantity not Stock
    Given I log in as Admin with username "admin" and password "admin123"
    When I navigate to the plants list page
    Then validate the fourth column header in the plants table should be "Quantity"
